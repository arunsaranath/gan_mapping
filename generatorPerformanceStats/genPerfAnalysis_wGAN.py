...
from ganModels.allGANModels import generatorsGANModels, discriminatorsGANModels
...
class genPerfAnalysis():
    def __init__(self, micaLibName='/Volume1/data/CRISM/arun/micaSli_2_17_2020/UMass_micaExemplars.sli'):
        try:
            if not micaLibName:
                raise IOError
            else:
                ...
                crmica_data = preprocessing.minmax_scale(crmica_data.T).T
    ...
    def initGANModel(self, nBands=240, nGenFilters=400, nDisFilters=25, filterSize=11, genWeightsLoc='',
                     disWeightsLoc=''):
        """
        ...
        dis1 = discriminatorsGANModels().ganModels_Dis1(ganType="wGAN")
        if disWeightsLoc:
    ...
    def createFeatureModel(self, nBands=240, nDisFilters=25, filterSize=11):
        """
        This function will generate a feature extractor with only the penultimate layer of the discriminator--assuming
        it is using my model.
        ----------------------------------------------------------------------------------------------------------------
        ...
        model_l2.add(Conv1D(filters=nDisFilters, kernel_size=filterSize, strides=2, input_shape=(nBands, 1),
                            weights=self.dis1.layers[0].get_weights(), padding='same'))
        # model_l2.add(Dropout(0.5))
        'Layer-2'
        'In: 120 X 1, depth =25'
        'Out: 60 X 1, depth =50'
        model_l2.add(Conv1D(filters=2*nDisFilters, kernel_size=filterSize, strides=2,
                            weights=self.dis1.layers[4].get_weights(), padding='same'))

        'Layer-3'
        'In: 60 X 1, depth =50'
        'Out: 30 X 1, depth =100'
        model_l2.add(Conv1D(filters=4*nDisFilters, kernel_size=filterSize, strides=2,
                            weights=self.dis1.layers[8].get_weights(), padding='same'))

        'Layer-4'
        'In: 30 X 1, depth =100'
        'Out: 15 X 1, depth =200'
        model_l2.add(Conv1D(filters=8*nDisFilters, kernel_size=filterSize, strides=2,
                            weights=self.dis1.layers[12].get_weights(), padding='same'))
    ...
    def plot_grouped_by_mica(self, sampleMat, similarity):
        ...
        for endmember, sample_idxs in groups.items():
            print('Processing {}'.format(self.spectraName[endmember]))
            n=0
            avg_sim = 0.
            ...
            if n>=1:
                ...
                plt.savefig(('/Volume2/arunFiles/python_HSITools/generatorPerformanceStats/balWGANGenPerf/MICASpectra/'+'mica_{}_{}.png'.format(endmember, filename)))
                plt.close()
    ...
    def plot_training_hist(self, similarity):
        ...
        similarity[similarity < np.cos(np.pi /4.)] = 0
        similarity[similarity < 0.85] = 0
        ...
        fig.tight_layout
        plt.savefig(('/Volume2/arunFiles/python_HSITools/generatorPerformanceStats/balWGANGenPerf/'+'trainHist_m5.png'),
                    bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    ...
    '------------------------------------------------------------------------------------------------------------------'
    'Create the models'
    obj1 =  genPerfAnalysis()
    'The weights locations are'
    gen1Loc = '/Volume2/arunFiles/python_HSITools/wGAN/Model_2020-04-26_18:53:42/ganWeights/generator/gen_cR_99.h5'
    dis1Loc = '/Volume2/arunFiles/python_HSITools/wGAN/Model_2020-04-26_18:53:42/ganWeights/discriminator/dis_cR_99.h5'
    'Get the models for this architecture'
    obj1.initGANModel(genWeightsLoc=gen1Loc, disWeightsLoc=dis1Loc)
    ...
    genSpectra = preprocessing.minmax_scale(genSpectra.T).T

    #fig1 = plt.figure()
    #plt.plot(range(240), np.squeeze(genSpectra[0, :]))
