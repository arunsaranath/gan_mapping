...
class genPerfAnalysis():
    ...
    def createFeatureModel(self, nBands=240, nDisFilters=20, filterSize=11):
        """
        ...
        model_l2.add(Conv1D(filters=nDisFilters, kernel_size=filterSize, strides=2, input_shape=(nBands, 1),
                            weights=self.dis1.layers[0].get_weights(), padding='same', activation='relu'))
        ...
        model_l2.add(Conv1D(filters=4*nDisFilters, kernel_size=filterSize, strides=2,
                            weights=self.dis1.layers[4].get_weights(), padding='same', activation='relu'))

        'Layer-4'
        'In: 30 X 1, depth =100'
        'Out: 15 X 1, depth =200'
        model_l2.add(Conv1D(filters=8*nDisFilters, kernel_size=filterSize, strides=2,
                            weights=self.dis1.layers[6].get_weights(), padding='same', activation='relu'))
    ...
    def plot_grouped_by_mica(self, sampleMat, similarity):
        ...
        for endmember, sample_idxs in groups.items():
            ...
            if n>=1:
                ...
                plt.savefig(('/Volume2/arunFiles/python_HSITools/generatorPerformanceStats/MICASpectra/'+'mica_{}_{}.png'.format(endmember, filename)))
                plt.close()
    ...
    def plot_training_hist(self, similarity):
        ...
        fig.tight_layout
        plt.savefig(('/Volume2/arunFiles/python_HSITools/generatorPerformanceStats/MICASpectra/'+'trainHist.png'),
                    bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    ...
    gen1Loc = '/Volume2/arunFiles/pythonCodeFiles/CRISM_repLearning/modelsOnYukiModel_cR_wGAN/Models/Model-4_small'+\
              '/generator/gen_cR_75.h5'
    dis1Loc = '/Volume2/arunFiles/pythonCodeFiles/CRISM_repLearning/modelsOnYukiModel_cR_wGAN/Models/Model-4_small'+\
