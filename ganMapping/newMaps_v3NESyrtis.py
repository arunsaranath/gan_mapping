...
if __name__ == "__main__":
    ...
    srcFolder =  "/Volume2/data/CRISM/AMS/v3_NESyrtis/"

    'Set the threshold levels for the various classes'
    threshMatrix = [[0.95, 0.975], [0.8, 0.95], [0.95, 0.97], [0.83, 0.88], [0.85, 0.95], [0.85, 0.95], [0.85, 0.95],
                    [0.90, 0.97], [0.85, 0.95], [0.85, 0.95], [0.85, 0.95], [0.9, 0.975], [0.85, 0.95], [0.85, 0.95],
                    [0.85, 0.95], [0.85, 0.95], [0.85, 0.95], [0.85, 0.95], [0.95, 0.975], [0.85, 0.95], [0.85, 0.975],
                    [0.85, 0.95], [0.85, 0.95], [0.85, 0.95], [0.96, 0.98], [0.85, 0.95], [0.85, 0.95]]
    threshMatrix = np.asarray(threshMatrix)

    'Set the colors for the various classes'
    colMat= [[255, 0, 0], [255, 0, 0], [0, 255, 0], [0, 255, 0], [218, 165, 32], [255, 255, 0], [255, 0, 255],
             ...
             [255, 128, 0], [255, 128, 0], [255, 128, 0], [0, 0, 255], [128, 0, 0], [0, 204, 102]]
    colMat = np.asarray(colMat)
    imgName=''

    for r, d, f in os.walk(srcFolder):
        for file in f:
            if ((file.find('_smoothed5_CR_micaMaps_Cosine.img') != -1)):
                'The image name is'
                ...
                bestGuessName = imgName.replace('_Cosine', '_BestGuess_final')
    ...
    for r, d, f in os.walk(srcFolder):
        for file in f:
            if file.find('_smoothed5_CR_micaMaps_BestGuess_final.img') != -1:
                'Create Composite bestGuess Image'
                bestGuessName = os.path.join(r, file)

                bestGuessHdrName = bestGuessName.replace('.img', '.hdr')
                img = envi.open(bestGuessHdrName, bestGuessName)
                cube = img.load()
                ...
                classMapName = bestGuessName.replace('BestGuess', 'IdentMap')
                ...
                classMapName = bestGuessName.replace('BestGuess', 'GuessMap')
                classMapName = classMapName.replace('.img', '.hdr')
                envi.save_image(classMapName, finalColMap, dtype=np.float32,
                                force=True, interleave='bil')
                ...
                detMapName = bestGuessName.replace('BestGuess', 'DetectionMap')
