...
if __name__ == "__main__":
    ...
    srcFolder = "/Volume1/data/CRISM/yuki/sabcondv4/nesyrtis/"
    trgtFolder = "/Volume2/data/CRISM/AMS/sabcondv4/nesyrtis/"
    sLvl = 0.1
    scaleFlag = True
        ...
        for file in f:
            if file.find('_nr.img') != -1:
                ...
                for kernelSize in range(1,6,2):
                    ...
                    '--------------------------------------------------------------------------------------------------'
                    crSmoothImageName = modelFitTools(4, 240, imgType=file[:3]).crismImgSmooth(crInterpImageName, kernelSize)
                ...
                colMat = [[255, 0, 0], [255, 0, 0], [0, 255, 0], [0, 255, 0], [218, 165, 32], [189, 183, 107],
                          [255, 255, 0], [255, 0, 255], [0, 0, 0], [139, 69, 19], [65, 105, 225], [0, 255, 255],
                          [0, 255, 255], [240, 255, 255], [0, 128, 128], [255, 255, 0], [0, 0, 0], [0, 0, 0],
                          [0, 206, 203], [192, 192, 192], [255, 165, 0], [0, 0, 0], [255, 160, 122], [128, 128, 0],
                          [255, 0, 255]]
                colMat = np.asarray(colMat, dtype=np.float32)

                'Initialize the low confidence bands for which we are using higher thresholds'
                lowConfBands = [0, 7, 8, 19, 22, 23, 24]
                'Generate the identification map'
                identMap = minMapTools(4, 240, imgType=file[:3]).create_Maps4CRISMImages_IdentMap(compMapName, colMat, identLevel=0.95,
                                                                                lowConfidenceMins=lowConfBands)
                'Generate the Guess map'
                guessMap = minMapTools(4, 240, imgType=file[:3]).create_Maps4CRISMImages_GuessMap(compMapName, colMat, identLevel=0.95,
                                                                                guessLevel=0.85,
                                                                                lowConfidenceMins=lowConfBands)
