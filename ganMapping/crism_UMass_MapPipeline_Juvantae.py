...
if __name__ == "__main__":
    ...
    srcFolder = "/Volume1/data/CRISM/yuki/v3_Juventae/"
    trgtFolder = "/Volume2/data/CRISM/AMS/v3_Juventae/"
    sLvl = 0.1
    scaleFlag = True
    ...
    '------------------------------------------------------------------------------------------------------'
    sliName = '/Volume1/data/CRISM/arun/juventaeSli_11_01_2020/UMass_juventaeExemplars.sli'
    sliHdrName = '/Volume1/data/CRISM/arun/juventaeSli_11_01_2020/UMass_juventaeExemplars.hdr'
    ...
    for r, d, f in os.walk(srcFolder):
        for file in f:
            if file.find('_woice_nr_ds.img') != -1:
                'The image name is'
                ...
                '------------------------------------------------------------------------------------------------------'
                abImgName = imgName.replace('_nr_', '_AB_')
                ...
                for kernelSize in range(1,6,2):
                    ...
                    maskName = minMapTools(4, 240).create_Mask4CRISMImages(crSmoothImageName, sigAbsoprtionLevel=0.994)
                    'Now create the best guess map'
                    bestGuessName = minMapTools(4, 240).create_Maps4CRISMImages_BestGuess(simScoreMapName)

                '------------------------------------------------------------------------------------------------------'
                'STEP-9: COMBINE ALL THE BEST GUESS MAPS TO CREATE A COMPOSITE BEST GUESS MAP'
                '------------------------------------------------------------------------------------------------------'
                compMapName = minMapTools(4, 240).create_Maps4CRISMImages_CompBestGuess(bestGuessName)
                ...
                colMat = [[255, 0, 0], [255, 0, 0], [0, 255, 0], [0, 255, 0], [218, 165, 32], [189, 183, 107],
                          [210, 105, 30], [255, 0, 255], [0, 0, 0], [112, 128, 144], [65, 105, 225], [0, 255, 255],
                          [0, 255, 255], [240, 255, 255], [0, 128, 128], [255, 255, 0], [0, 0, 0], [0, 0, 0],
                          [0, 206, 203], [192, 192, 192], [255, 165, 0], [240, 230, 140], [240, 230, 140],
                          [107, 142, 35], [72,209, 204], [255, 165, 0], [255, 165, 0]]
                colMat = np.asarray(colMat, dtype=np.float32)

                'Initialize the low confidence bands for which we are using higher thresholds'
                lowConfBands = [0, 7, 8, 20, 26, 25, 24, 23, 22, 21]
                'Generate the identification map'
                identMap = minMapTools(4, 240).create_Maps4CRISMImages_IdentMap(compMapName, colMat, identLevel=0.95,
                                                                                lowConfidenceMins=lowConfBands)
                'Generate the Guess map'
                guessMap = minMapTools(4, 240).create_Maps4CRISMImages_GuessMap(compMapName, colMat, identLevel=0.95,
                                                                                guessLevel=0.85,
                                                                                lowConfidenceMins=lowConfBands)
